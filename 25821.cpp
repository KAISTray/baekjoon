...
int main() {
	...
	for (int i = 1; i < 7; i++) {
		for (int j = pow10int(i - 1); j <= expand(i, 9); j++) {
			ll S = ((ll)(j - (j % 10)) * (ll)pow10int(i - 1)) + (ll)flip(j);
			//printf("t %lld %lld %lld\n", (j - (j % 10)), pow10int(i - 1), flip(j));
			//printf("t %lld\n", S);
			if (S < 2) {
				continue;
			}
	...
	int count = upper_bound(p_list.begin(), p_list.end(), H) - lower_bound(p_list.begin(), p_list.end(), L);
	printf("%d", count);
}
