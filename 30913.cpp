...
1. a^e = 1 (mod N)인 N 찾기 (order 찾기)
N 소인수분해 갈기고 Lcm 취하기 
-> Euler phi 구하고 약수 돌려보기 

2. a^e = 1 (mod N)
primitive root g -> g^k = 1 (mod N)
ord(g)_N = e -> ord(g^d) => e / (gcd(d, e));
gcd(d, e) = 1인 d -> 
...
ll Query1(ll a, ll N) {
	...
	ll phi = Euler_Phi(N);
	vector<ll> fac = factor_vector(phi);
	for (vector<ll>::iterator i = fac.begin(); i != fac.end(); i++) {
		if (M_Power(a, *i, N) == 1) {
			return *i;
		}
	}
